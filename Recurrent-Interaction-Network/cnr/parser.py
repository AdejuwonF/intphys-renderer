...
def parse_args(argv=None):
    ...
    parser.set_defaults(
        model="PRN",
        checkpoint=Path(
            "/home/adejuwon/derenderer/Recurrent-Interaction-Network/checkpoints"
        ),
        batch_size=8,
    )
    parser.add_argument("--model", default="PRN", help="model to train")
    parser.add_argument("--upsample_mode", default="bilinear")
    parser.add_argument("--dataset", default="Dataset_intphys")
    parser.add_argument(
        "--input_prefix",
        type=Path,
        default=Path("/disk1/intphys_data/train"),
    )
    # parser.add_argument("--item_list", nargs="+", default=[])
    parser.add_argument(
        "--item_list", nargs="+", default=["image_array", "semantic_mask", "depth"]
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "/home/adejuwon/derenderer/Recurrent-Interaction-Network/datasets/metadata.json"
        ),
    )
    parser.add_argument("--visibility", type=str, default=None)
    parser.add_argument("--movement", type=str, default=None)
    parser.add_argument("--nobjects", type=str, default=None)
    parser.add_argument("--block", type=list, default= [1, 2, 3])
    parser.add_argument("--video_slice", type=slice, default=slice(None))
