...
class Core_CNR(nn.Module):
    def __init__(self, args):
        ...
        self.meshgrid = torch.nn.Parameter(make_meshgrid(
        ...
        self.initial_transform = nn.Sequential(
                nn.Linear(args.attr_len, 256),
                nn.ReLU(inplace=True),
                nn.Linear(256, args.nc_in),
                nn.ReLU(inplace=True)
                )
    ...
    def forward(self, x, n_obj):
        x = [x[i][:n_obj[i]] for i in range(x.shape[0])]
        x = [self.initial_transform(attr) for attr in x]
    ...
    def to(self, device):
        super().to(device)
        self.meshgrid = self.meshgrid.to(device)
        self.device = device
        return self
