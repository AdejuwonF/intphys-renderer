...
class Stepper(object):
    def __init__(self, cfg):
        ...
        if self.version == "paper-adept":
            self.sim_time_step, self.num_steps = 1.0/100, 4
        elif self.version == "intphys-adept":
            self.sim_time_step, self.num_steps = 1.0 / 30, 1
        else:
    ...
    def step(self, objects):
        ...
        objects = step(objects, self.sim_time_step, self.num_steps)
    ...
    def reverse_step(self, objects):
        ...
        if self.to_perturb:
            objects = self._perturb(objects)
        objects = reverse_step(objects, self.sim_time_step, self.num_steps)
        return objects
        ...
        for obj in objects:
            ...
            if obj["type"] != "occluder":
                ...
                if rand > 1 - self.stop_probability:
                    ...
                    obj["velocity"] = [0, 0, 0]
                # if self.version == "paper-adept" and obj['location'][0] < -2 or obj['location'][0] > .5:
                #     magic_penalty += 1000 #TODO: add for paper-ADEPT
