...
from .physics.utils import iou
from .match import Matcher
from .step import Stepper
from .physics.loss import build_loss
from .physics.visualize.mask_visualize import visible_area, visible_mask, visible_image
        ...
        for i in range(n):  # iterate through observations (including null registers)
            ...
            if i < n_observation_objects:  # observed object
                ...
                if matched_belief_id >= n_belief_objects:
                    ...
                    elif self.version == "intphys-adept":
    ...
    def _handle_unmatched_observation(self, obj):
        if self.version == "intphys-adept":
            ...
            for t in range(self.t - 5, 0, -5):
                ...
                if mask_area > self.area_threshold and t < self.t - 15:
                    score += self.unmatched_observation_penalty * mask_area
                if score > self.unmatched_observation_max_penalty:
                    break
            self.score += score
        elif self.version == "paper-adept":
            obj_state = [o for o in self.belief if o['type'] != 'occluder']
            obj_state.append(obj)
            magic_penalty = 0
...
class FilterUpdater(object):
    def __init__(self, cfg, belief, camera, n_filter):
        self.t = 0
        self.cfg = cfg
        self.belief = deepcopy(belief)
        self.camera = camera
        ...
        self.logger = logging.getLogger("adept_physics")
    ...
    def step_and_update(self, observation):
        if self.n_filter.acquire(False):
            ...
            # if self.cfg.DEBUG:
            particles_lists = [FilterUpdater._combined_step_and_update(*w) for w in worker_args]
            # else:
    ...
    def run(self, observations):
        iterator = enumerate(tqdm(observations)) if self.cfg.DEBUG else enumerate(observations)
        # for i, observation in enumerate(tqdm(observations)):
        # for i, observation in enumerate(observations):
        for i, observation in iterator:
